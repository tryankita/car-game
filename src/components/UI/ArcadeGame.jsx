...
export default function ArcadeGame() {
  ...
  useEffect(() => {
    ...
    async function initGame() {
      try {
        const { NeonRushGame } = await import('../../arcade/Game.js')
        game = new NeonRushGame(() => goHome())
        gameRef.current = game
      } catch (err) {
        console.error('Arcade game init failed:', err)
      }
    }
  ...
  return (
    <div id="arcade-root">
      ...
      <div id="start-screen" className="hidden">
        ...
        <button id="maps-button" style={{ marginTop: 20 }}>MAPS</button>
        <div id="selected-map-display">MAP: CITY</div>
        <div className="controls-hint">
          ...
          <div className="key-hint">
            <div className="key" style={{ width: 80 }}>SPACE</div>
            <span>PAUSE</span>
          </div>
        </div>
      </div>
      ...
      {/* Map Selector */}
      <div id="map-selector-overlay" className="hidden">
        <h1 className="main-title" style={{ fontSize: 'clamp(1.5rem,5vw,3rem)', marginBottom: 20 }}>SELECT TRACK</h1>
        <div className="map-carousel">
          <div className="map-card active" data-map="city">
            <div className="map-card-bg city-preview"></div>
            <div className="map-card-content"><h3>CENTRAL CITY</h3><p>Urban Streets</p></div>
          </div>
          <div className="map-card" data-map="cybercity">
            <div className="map-card-bg cyber-preview"></div>
            <div className="map-card-content"><h3>CYBER PUNK</h3><p>Neon &amp; Rain</p></div>
          </div>
          <div className="map-card" data-map="desert">
            <div className="map-card-bg desert-preview"></div>
            <div className="map-card-content"><h3>SCORCHED</h3><p>Hot Sands</p></div>
          </div>
          <div className="map-card" data-map="snow">
            <div className="map-card-bg snow-preview"></div>
            <div className="map-card-content"><h3>FROST PEAK</h3><p>Ice &amp; Snow</p></div>
          </div>
        </div>
        <button id="close-map-selector" className="modal-button" style={{ marginTop: 30, padding: '15px 50px', background: '#fff', color: '#2d3436', width: 'auto', maxWidth: 250 }}>BACK</button>
      </div>

      {/* Three.js canvas mount point */}
      <div id="arcade-container"></div>

      {/* Speed / nitro overlays */}
      ...
      <div id="settings-modal" className="modal">
        ...
        <div className="setting-row">
          ...
          <input type="range" id="volume-slider" min="0" max="1" step="0.1" defaultValue="0.5" />
        </div>
        <div className="setting-row" style={{ marginTop: 5 }}>
          <span className="setting-label">Camera View</span>
          <div className="camera-options">
            <div className="camera-tile active" data-mode="normal" id="cam-normal">
              <div className="camera-preview"><img src="/arcade-img/BEHIND.png" alt="Behind View" /></div>
              <span>BEHIND</span>
            </div>
            <div className="camera-tile" data-mode="driver" id="cam-driver">
              <div className="camera-preview"><img src="/arcade-img/DRIVER.png" alt="Driver View" /></div>
              <span>DRIVER</span>
            </div>
            <div className="camera-tile" data-mode="top" id="cam-top">
              <div className="camera-preview"><img src="/arcade-img/TOP.png" alt="Top View" /></div>
              <span>TOP</span>
            </div>
          </div>
        </div>
        <div className="setting-row" id="steering-mode-row" style={{ marginTop: 5 }}>
          <span className="setting-label">Steering Mode</span>
          <div className="control-options">
            <div className="control-tile active" data-mode="buttons" id="ctrl-buttons"><span>BUTTONS</span></div>
            <div className="control-tile" data-mode="swipe" id="ctrl-swipe"><span>SWIPE</span></div>
            <div className="control-tile" data-mode="tilt" id="ctrl-tilt"><span>TILT</span></div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
          <button id="close-settings" className="modal-button" style={{ flex: 2 }}>BACK</button>
          <button id="home-button-settings" className="modal-button" style={{ flex: 1, background: '#74b9ff', fontSize: '0.7em', boxShadow: '0 8px 0px #0984e3' }}>HOME</button>
        </div>
      </div>

      {/* Mobile steer buttons */}
      <div id="left-button" className="control-button">◀</div>
      <div id="right-button" className="control-button">▶</div>
    </div>
  )
}
